$(window).load(function(){
    ...
    var product_list_width = $product_list.width(); //ul의 너비
    var li = $('#product_list').children(); //li
    var li_width = li.outerWidth(true); //li의 너비
    var img_number = $('#product_list').children().size(); //li의 갯수

    var current_position; //현재위치

    var probtn = $('.arrow_btn #pro_prevbtn a'); //이전버튼,

    var nextbtn = $('.arrow_btn #pro_nextbtn a'); //다음버튼
    ...
    function pro_prev(e){ //이전버튼 
        e.preventDefault();
        current_position = $product_list.position().left ;
        //현재위치 = ul의 position().left값
        ...
        // ul의 애니메이션상태가 아니라면, ul의 current_position에 li_width값을 더해서 움직임(왼쪽으로)
        //그리고 ul의 position값을 초기화함

    }


    function pro_next(e){ //다음버튼 
        e.preventDefault();
        current_position = $product_list.position().left ;
        //현재위치 = ul의 position().left값
        $product_list.not(':animated').animate({
        ...
        // ul의 애니메이션상태가 아니라면, ul의 current_position에 li_width값을 빼서 움직임(오른쪽으로)
        //그리고 ul의 position값을 초기화함
